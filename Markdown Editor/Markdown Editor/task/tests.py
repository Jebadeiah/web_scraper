...
class Test(StageTest):
    ...
    def check_result_in_file(self, attach):
        try:
            with open('output.md', 'r', encoding='utf-8') as outfile:
                try:
                    output = outfile.read()
                except UnicodeDecodeError:
                    raise WrongAnswer("An error occurred while reading your file.\n"
                                      "Please, make sure you output your file in utf-8 encoding.")
    ...
    def test4(self):
        ...
        output = list(map(lambda item: item.lower(), pr.execute('line-break').split('\n')))
        if len(output) != 3 or output[1] != '':
            return CheckResult.wrong(
                'Line-break formatter only moves the input pointer to the next line, and prompts a user for a new formatter')
